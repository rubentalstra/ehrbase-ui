...
15. **No machine identifier is a user-facing handle.** A UUID / `ehr_id` / composition `uid` / `version_uid` is **never typed by a user** and is **never the primary thing shown** for identification or navigation entry — nobody remembers or can obtain a UUID. Patients are found and verified by **name + date of birth + MRN** (the short human record number); the `ehr_id` is resolved server-side from `EHR_STATUS.subject.external_ref` (openEHR EHR/Demographic separation; EHRbase `GET /ehr?subject_id&subject_namespace`; FHIR MRN type `MR`; IHE PDQ; ADR-0046). Internal ids MAY appear in URL paths (reached via search/links, never typed) and as secondary, copyable "technical details" only — never as a required input or a search key. Patient search/identify/navigate go through the global search (⌘K + `/patients`) + the `PatientPicker`; new EHR-scoped surfaces operate inside a selected patient (the patient-context shell), not a pasted id. The `clinical-ui-reviewer` sub-agent enforces.
