...
> **Description:** The missing open-source UI for EHRbase. Clinical workspace, dynamic openEHR forms, AQL query builder. TanStack Start + React 19 + shadcn/ui + Keycloak. Built for EU clinical deployments. _(This document still describes the comprehensive GDPR audit-log schema — ISO 27799 baseline, NEN 7513 in NL among the national standards checked — as part of the v1.0 **target**. That layer was removed in the 2026-05-30 core-refocus and is **deferred post-core**: it is not in the current build and returns before any deployment touches real patient data. See CLAUDE.md → "Deferred (post-core)" and the ❌ REMOVED markers on the §13/§14 chapters below.)_
> **License:** Apache-2.0
