...
// Namespace-level mock proxy — accessing `.ping` through the mocked() proxy is
// a field read, not a method reference, so the no-unbound-method rule doesn't
// fire (it would fire on `vi.mocked(valkey.ping)`, which pulls the method off
// the instance). The DB `execute` mocks use the hoisted handles above instead.
